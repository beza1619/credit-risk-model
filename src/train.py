...
def load_and_prepare_data(data_path):
    ...
    print(f"      High Risk (1): {(target == 1).sum()} ({(target == 1).mean()*100:.1f}%)")
    print(f"      Low Risk (0): {(target == 0).sum()} ({(target == 0).mean()*100:.1f}%)")
...
def create_preprocessor():
    ...
    numeric_features = ['transaction_count', 'total_amount', 'avg_amount', 'std_amount',
                       ...
                       'frequency', 'monetary', 'avg_transaction_value', 'transaction_std',
                       'provider_diversity', 'product_diversity', 'channel_diversity',
                       'amount_range', 'monetary_per_day']
...
def plot_roc_curve(y_true, y_pred_proba, model_name):
    ...
    plt.plot(fpr, tpr, color='darkorange', lw=2, label=f'ROC curve (AUC = {roc_auc_score(y_true, y_pred_proba):.4f})')
...
def main(fast_mode=True):
    ...
    if fast_mode:
        ...
        lr_metrics = evaluate_model(lr_pipeline, X_test, y_test, "Logistic Regression (Simple)")
        ...
        rf_metrics = evaluate_model(rf_pipeline, X_test, y_test, "Random Forest (Simple)")
    ...
    best_model_name = max(models_trained.keys(), 
                         key=lambda x: models_trained[x][1]['roc_auc'])
