...
class RFMCalculator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        rfm["transaction_std"] = (
            df.groupby(self.customer_col)[self.amount_col].std().values
        )
...
class DataProcessor:  # Changed from FeatureEngineer to DataProcessor for simplicity
    ...
    def create_customer_features(self, df):
        ...
        if "ProviderId" in df.columns:
            provider_div = (
                df.groupby("CustomerId")["ProviderId"].nunique().reset_index()
            )
        ...
        if "ProductCategory" in df.columns:
            product_div = (
                df.groupby("CustomerId")["ProductCategory"].nunique().reset_index()
            )
        ...
        all_features = pd.merge(
            all_features, rfm_features, on="customer_id", how="left"
        )
        ...
        all_features["amount_range"] = (
            all_features["max_amount"] - all_features["min_amount"]
        )
        all_features["monetary_per_day"] = all_features["monetary"] / (
            all_features["recency"] + 1
        )
